--[[
    caelChat - Modern WoW Chat Customization Addon (Refactored for modern WoW APIs)
    
    Modern API Changes:
    - UIDropDownMenu system replaced with MenuUtil.CreateContextMenu (modern context menu API)
    - OnUpdate delay loops replaced with C_Timer.After (modern timer API)
    - Streamlined event handler structure
    - Removed deprecated globals usage where possible
]]
...
local function KillTextures(chatName)
    ...
    -- Set a blank backdrop to override Blizzard textures
    local blankBackdrop = {
        bgFile = nil,
        edgeFile = nil,
        tile = false,
    ...
    -- Apply blank backdrop to main frame
    frame:SetBackdrop(blankBackdrop)
    frame:SetBackdropColor(0, 0, 0, 0)
    frame:SetBackdropBorderColor(0, 0, 0, 0)

    -- Strip all child regions recursively
    local function StripFrameTextures(f)
        if not f then return end
        
        -- Hide all regions (textures, etc)
    ...
    -- Strip main chat frame
    StripFrameTextures(frame)

    -- Strip chat tab frame and its children
    local tabFrame = _G[chatTabName]
    if tabFrame then
        StripFrameTextures(tabFrame)
        tabFrame:Hide()
    end

    -- Strip button frame and all buttons
    local btnFrame = _G[chatButtonFrame]
    if btnFrame then
        StripFrameTextures(btnFrame)
        btnFrame:Hide()
        for _, buttonName in pairs({"UpButton", "DownButton", "BottomButton", "MinimizeButton"}) do
            local btn = _G[chatButtonFrame .. buttonName]
            if btn then
                StripFrameTextures(btn)
                btn:Hide()
            end
        end
    end
    ...
    -- Strip edit box
    local editBox = _G[chatEditBoxName]
    if editBox then
        StripFrameTextures(editBox)
        if editBox.SetAltArrowKeyMode then
            editBox:SetAltArrowKeyMode(false)
        end
    end

    -- Hook to continuously strip textures if Blizzard recreates them
    if not frame.textureSentinelHooked then
        hooksecurefunc(frame, "Show", function(self)
            StripFrameTextures(self)
            local tab = _G[self:GetName() .. "Tab"]
            if tab then StripFrameTextures(tab) end
        end)
        frame.textureSentinelHooked = true
    end
end
...
-- Handle color changes to the chatbox edit frame panel (caelPanel_EditBox)
-- Using hooksecurefunc for secure hook into chat system
...
-- Modern context menu for chat tab options
local function ShowChatTabContextMenu(chatButton)
    local menu = {
        {
            text = "Config",
            func = function()
        ...
        },
        {
            ...
            submenu = function()
                local submenu = {}
                local frameID = chatButton.id
                local chatFrame = _G[format("ChatFrame%s", frameID)]
                local _, currentSize = chatFrame:GetFont()
                
                for _, size in pairs(CHAT_FONT_HEIGHTS) do
                    table.insert(submenu, {
                        text = format(FONT_SIZE_TEMPLATE, size),
                        func = function()
    ...
    MenuUtil.CreateContextMenu(UIParent, menu)
end
...
-- Initialize chat colors and other settings on world enter
local function InitializeChatColors()
...
caelChat.eventFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_ENTERING_WORLD" then
        -- Use modern timer API instead of OnUpdate loop
        C_Timer.After(10, InitializeChatColors)
    end
end)
