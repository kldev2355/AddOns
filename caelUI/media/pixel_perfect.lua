...
-- Our screen width and height with safe fallback values
local screen_width = "1920"
local screen_height = "1080"

-- Try to get actual screen size if available
do
    local success, res_width, res_height = pcall(GetPhysicalScreenSize)
    if success and res_width and res_height then
        screen_width = tostring(res_width)
        screen_height = tostring(res_height)
    end
end
...
function private.set_scale (...)
    ...
    if ui_scale ~= -1 then
        ...
        local current_height = 1080
        local success, _, res_height = pcall(GetPhysicalScreenSize)
        if success and res_height then
            current_height = res_height
        end
        scale_fix = (768 / current_height) / ui_scale
    end
end
