...
media.files = {
    ...
    statusbar_a             = media.directory .. "statusbars/normtexa",
    statusbar_b             = media.directory .. "statusbars/normtexb",
    statusbar_c             = media.directory .. "statusbars/normtexc",
    statusbar_d             = media.directory .. "statusbars/normtexd",
    statusbar_e             = media.directory .. "statusbars/normtexe",
    statusbar_f             = media.directory .. "statusbars/normtexf",
    statusbar_g             = media.directory .. "statusbars/normtexg",
...
function media.create_backdrop (self, name)
    local parent
    local selfType = self:GetObjectType()
    ...
    if selfType == "Frame" or selfType == "StatusBar" then
        parent = self
    elseif self:GetParent() then
        local parentType = self:GetParent():GetObjectType()
        if parentType == "Frame" or parentType == "StatusBar" then
    ...
    self.backdrop = CreateFrame("Frame", name and name .. "Backdrop" or nil, parent, "BackdropTemplate")
...
function media.create_blank_backdrop (parent, name)
    local overlay = CreateFrame("Frame", name and name or nil, parent, "BackdropTemplate")
    ...
    overlay:SetBackdrop({
        bgFile = media.directory .. "borders/blank",
        edgeFile = media.directory .. "borders/blank",
...
function media.create_shadow (parent, name)
    local shadow = CreateFrame("Frame", name and name or nil, parent, "BackdropTemplate")
    ...
    shadow:SetBackdrop({
        edgeFile = media.directory .. "borders/glowTex1", edgeSize = PixelScale(3),
