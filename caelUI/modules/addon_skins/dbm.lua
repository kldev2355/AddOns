    ...
    for bar in self:GetBarIterator() do
        if not bar.injected then
            ...
            if not icon1.overlay then
                icon1.overlay = CreateFrame("Frame", "$parentIcon1Overlay", frame_bar, "BackdropTemplate")
                ...
                if Draw_Shadows and not icon1.overlay.shadow then
                    local shadow = CreateFrame("Frame", nil, icon1.overlay, "BackdropTemplate")
            ...
            if not icon2.overlay then
                icon2.overlay = CreateFrame("Frame", "$parentIcon2Overlay", frame_bar, "BackdropTemplate")
                ...
                if Draw_Shadows and not icon2.overlay.shadow then
                    local shadow = CreateFrame("Frame", nil, icon2.overlay, "BackdropTemplate")
                    shadow:SetFrameLevel(1)
                    shadow:SetFrameStrata("BACKGROUND")
                    shadow:SetPoint("TOPLEFT", -PixelScale(3), PixelScale(3))
                    shadow:SetPoint("BOTTOMLEFT", -PixelScale(3), -PixelScale(3))
            ...
            if not frame.styled then
                ...
                if Draw_Shadows and not frame.shadow then
                    local shadow = CreateFrame("Frame", nil, frame.overlay, "BackdropTemplate")
                    shadow:SetFrameLevel(1)
                    shadow:SetFrameStrata("BACKGROUND")
                    shadow:SetPoint("TOPLEFT", -PixelScale(3), PixelScale(3))
                    shadow:SetPoint("BOTTOMLEFT", -PixelScale(3), -PixelScale(3))
    ...
    while (_G[format("DBM_BossHealth_Bar_%d", count)]) do
        ...
        if not bar.styled then
            ...
            if not bar.SetBackdrop then
                pcall(Mixin, bar, BackdropTemplateMixin)
            end
            ...
            if Draw_Shadows and not bar.shadow then
                local shadow = CreateFrame("Frame", nil, bar.overlay, "BackdropTemplate")
                shadow:SetFrameLevel(1)
                shadow:SetFrameStrata("BACKGROUND")
                shadow:SetPoint("TOPLEFT", -PixelScale(3), PixelScale(3))
                shadow:SetPoint("BOTTOMLEFT", -PixelScale(3), -PixelScale(3))
...
DBMRangeCheck:HookScript("OnShow", function(self)
    if not self.SetBackdrop then
        pcall(Mixin, self, BackdropTemplateMixin)
    end
    ...
    if Draw_Shadows and not self.shadow then
        local shadow = CreateFrame("Frame", nil, self.overlay, "BackdropTemplate")
        shadow:SetFrameLevel(1)
        shadow:SetFrameStrata("BACKGROUND")
        shadow:SetPoint("TOPLEFT", -PixelScale(3), PixelScale(3))
        shadow:SetPoint("BOTTOMLEFT", -PixelScale(3), -PixelScale(3))
