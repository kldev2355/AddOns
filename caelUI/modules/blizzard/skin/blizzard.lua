...
SkinBlizzard:RegisterEvent("ADDON_LOADED", function(self, _, addon)
    ...
    for _, frame in pairs{
        ...
        if frame == "InterfaceOptionsFrameTab1" or frame == "InterfaceOptionsFrameTab2" then
            ...
            if frameObj then
                SkinButton(frameObj)
                frameObj:SetScript("OnEnter", SetModifiedBackdrop)
                frameObj:SetScript("OnLeave", SetOriginalBackdrop)
                frameObj:SetScript("OnShow", nil)
                
                local frameText = _G[frame .. "Text"]
                if frameText then
                    frameText:ClearAllPoints()
                    frameText:SetPoint("CENTER", frameObj, "CENTER")
                    frameText.SetPoint = function() return end
                end
            end
        end
    end
    ...
    for i = 1, getn(ChatMenus) do
        local chatMenu = _G[ChatMenus[i]]
        if chatMenu then
            if chatMenu == _G["ChatMenu"] then
                chatMenu:HookScript("OnShow", function(self) SkinPanel(self) self:ClearAllPoints() self:SetPoint("BOTTOMRIGHT", ChatFrame1, "BOTTOMRIGHT", 0, 30) end)
            else
                chatMenu:HookScript("OnShow", function(self) SkinPanel(self) end)
            end
        end
    end
    ...
    for _, button in next, BlizzardMenuButtons do
        ...
        if UIMenuButtons then
            ...
            for _, region in next, {"Left", "Right", "Middle"} do
                local texture = _G["GameMenuButton" .. button .. region]
    ...
    if _G["VideoOptionsFrameCancel"] and _G["VideoOptionsFrameApply"] and _G["VideoOptionsFrameOkay"] then
        _G["VideoOptionsFrameCancel"]:ClearAllPoints()
        _G["VideoOptionsFrameCancel"]:SetPoint("RIGHT", _G["VideoOptionsFrameApply"], "LEFT", -4, 0)     
        _G["VideoOptionsFrameOkay"]:ClearAllPoints()
        _G["VideoOptionsFrameOkay"]:SetPoint("RIGHT", _G["VideoOptionsFrameCancel"], "LEFT", -4, 0)
    end
    if _G["AudioOptionsFrameOkay"] and _G["AudioOptionsFrameCancel"] then
        _G["AudioOptionsFrameOkay"]:ClearAllPoints()
        _G["AudioOptionsFrameOkay"]:SetPoint("RIGHT", _G["AudioOptionsFrameCancel"], "LEFT", -4, 0)
    end
    if _G["InterfaceOptionsFrameOkay"] and _G["InterfaceOptionsFrameCancel"] then
        _G["InterfaceOptionsFrameOkay"]:ClearAllPoints()
        _G["InterfaceOptionsFrameOkay"]:SetPoint("RIGHT", _G["InterfaceOptionsFrameCancel"], "LEFT", -4, 0)
    end
    if _G["ColorPickerOkayButton"] and _G["ColorPickerFrame"] and _G["ColorPickerCancelButton"] then
    ...
    if _G["ReadyCheckFrameYesButton"] and _G["ReadyCheckFrameNoButton"] and _G["ReadyCheckFrame"] and _G["ReadyCheckFrameText"] then
    ...
    if _G["InterfaceOptionsFrameTab1"] and _G["InterfaceOptionsFrameCategories"] then
    ...
    if _G["InterfaceOptionsFrameTab2"] and _G["InterfaceOptionsFrameCategories"] then
